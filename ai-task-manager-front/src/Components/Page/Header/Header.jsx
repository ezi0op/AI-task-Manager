...
const Header = ({ workspaceMode, setWorkspaceMode }) => {
  ...
  return (
    <div className='flex justify-between items-center px-8 py-5 bg-white/50 backdrop-blur-sm border-b border-slate-100/60'>
      <div>
        <h1 className='text-2xl font-extrabold text-slate-800 tracking-tight'>{getPageTitle()}</h1>
        <p className='text-slate-400 mt-0.5 text-sm font-medium'>{getPageSubtext()}</p>
      </div>
      ...
      <div className='flex items-center gap-4'>
        ...
        {isGuest ? (
          <button
            ...
            className="flex items-center gap-2.5 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-indigo-700 transition-all shadow-md shadow-indigo-600/20"
          >
          ...
          >
            {profileImage && !headerImageError ? (
              <img 
                src={profileImage} 
                alt="Avatar" 
                className="h-10 w-10 rounded-xl object-cover shadow-md shadow-indigo-600/10 border border-slate-200"
                onError={() => {
                  setHeaderImageError(true);
                }}
              />
            ) : (
              <div className='h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex justify-center items-center shadow-md shadow-indigo-600/20'>
                <span className='text-white text-sm font-bold'>{capitalizedName.charAt(0)}</span>
              </div>
            )}
            <div className="flex flex-col">
              <span className='font-bold text-sm text-slate-800 leading-tight group-hover:text-indigo-600 transition-colors'>{capitalizedName}</span>
